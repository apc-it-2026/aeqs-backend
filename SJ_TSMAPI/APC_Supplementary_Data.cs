    ...
    class APC_Supplementary_Data
    {
        public static SJeMES_Framework_NETCore.WebAPI.ResultObject Commit_Main(object OBJ)
        {
            ...
            try
            {
                ...
                sql = $@"SELECT COUNT(1) FROM t_tsm_Files_upload_m WHERE FILE_NAME=@file_name";
                ...
                {
                    sql = $@"UPDATE t_tsm_Files_upload_m SET CURR_UPLOAD_TIME=TO_DATE(@currTime, 'yyyy-MM-dd HH24:mi:ss'),CURR_FILE_GUID=@file_guid,MODIFYBY=@modifyby,MODIFYDATE=@modifydate,MODIFYTIME=@modifytime WHERE FILE_NAME=@file_name";
                ...
                {
                    sql = $@"INSERT INTO t_tsm_Files_upload_m(FILE_NAME,CURR_UPLOAD_TIME,CURR_FILE_GUID,CREATEBY,CREATEDATE,CREATETIME) VALUES(@file_name,TO_DATE(@currTime, 'yyyy-MM-dd HH24:mi:ss'),@file_guid,@createby,@createdate,@createtime)";
                    paramsDic.Add("file_name", file_name);
                    paramsDic.Add("currTime", currTime.ToString("yyyy-MM-dd HH:mm:ss"));
                    paramsDic.Add("file_guid", file_guid);
                ...
                paramsDic = new Dictionary<string, object>();
                sql = $"INSERT INTO t_tsm_Files_upload(FILE_NAME,UPLOAD_TIME,FILE_GUID,CREATEBY,CREATEDATE,CREATETIME) VALUES(@file_name,TO_DATE(@currTime, 'yyyy-MM-dd HH24:mi:ss'),@file_guid,@createby,@createdate,@createtime)";
                paramsDic.Add("file_name", file_name);
                paramsDic.Add("currTime", currTime.ToString("yyyy-MM-dd HH:mm:ss"));
                paramsDic.Add("file_guid", file_guid);
                paramsDic.Add("createby", user);
        ...
        public static SJeMES_Framework_NETCore.WebAPI.ResultObject Main_Delete(object OBJ)
        {
            ...
            try
            {
                ...
                var paramsDic = new Dictionary<string, object>();
                paramsDic.Add("file_name", file_name);
                string sql = $@"delete from t_tsm_Files_upload where FILE_NAME=@file_name";
                DB.ExecuteNonQuery(sql, paramsDic);
                sql = $@"delete from t_tsm_Files_upload_m where FILE_NAME=@file_name";
                DB.ExecuteNonQuery(sql, paramsDic);
                DB.Commit();
                ret.IsSuccess = true;
                ret.ErrMsg = "Submitted successfully";
...
ORDER BY a.UPLOAD_TIME DESC";
        ...
        public static SJeMES_Framework_NETCore.WebAPI.ResultObject GetMainList(object OBJ)
        {
            ...
            try
            {
                ...
                {
                    where += $" AND File_name LIKE '%{Filename}%'";
                    //paramTestDic.Add("customer_no", $@"%{Filename}%");
                }
...
//        }
        ...
        public static SJeMES_Framework_NETCore.WebAPI.ResultObject DeleteBy(object OBJ)
        {
            SJeMES_Framework_NETCore.WebAPI.RequestObject ReqObj = (SJeMES_Framework_NETCore.WebAPI.RequestObject)OBJ;
            SJeMES_Framework_NETCore.WebAPI.ResultObject ret = new SJeMES_Framework_NETCore.WebAPI.ResultObject();
            SJeMES_Framework_NETCore.DBHelper.DataBase DB = new SJeMES_Framework_NETCore.DBHelper.DataBase();
            try
            {
                ...
                var paramsDic = new Dictionary<string, object>();
                paramsDic.Add("id", id);
                string sql = $@"delete from t_tsm_Files_upload where id=@id";
                DB.ExecuteNonQuery(sql, paramsDic);
                paramsDic = new Dictionary<string, object>();
                paramsDic.Add("file_name", file_name);
                sql = $@"
