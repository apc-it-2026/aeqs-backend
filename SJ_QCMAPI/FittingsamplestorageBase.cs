...
namespace SJ_QCMAPI
{
    public class FittingsamplestorageBase
    {
        /*PC端接口*****************************************/
        /// <summary>
        /// 新增试穿部样品存放管理（包括平板端新增）
        ...
        /// <returns></returns>
        public static SJeMES_Framework_NETCore.WebAPI.ResultObject FittingsamplestorageeAdd(object OBJ)
        {
            ...
            try
            {
                ...
                string sql = $@"select*from qcm_fittingsample_storage_m where item_no='{item_no}'";
                DataTable dt = DB.GetDataTable(sql);
                string sql2 = string.Empty;
                if (dt.Rows.Count > 0)
                {
                    sql2 = $"update qcm_fittingsample_storage_m set location_no='{location_no}',location_name='{location_name}' where item_no='{item_no}'";
                }
                else
                {
                    sql2 = $"insert into qcm_fittingsample_storage_m(location_no,location_name,item_no,item_name,vend_no,vend_name,prod_no,putin_date,end_date,createby,createdate,createtime) values('{location_no}','{location_name}','{item_no}','{item_name}','{vend_no}','{vend_name}','{prod_no}','{putin_date}','{end_date}','{CreactUserId}','{DateTime.Now.ToString("yyyy-MM-dd")}','{DateTime.Now.ToString("HH:mm:ss")}')";
                }
        ...
        /// <param name="OBJ"></param>
        /// <returns></returns>
        public static SJeMES_Framework_NETCore.WebAPI.ResultObject FittingsamplestorageUpdate(object OBJ)
        {
            SJeMES_Framework_NETCore.WebAPI.RequestObject ReqObj = (SJeMES_Framework_NETCore.WebAPI.RequestObject)OBJ;
            SJeMES_Framework_NETCore.WebAPI.ResultObject ret = new SJeMES_Framework_NETCore.WebAPI.ResultObject();
            SJeMES_Framework_NETCore.DBHelper.DataBase DB = new SJeMES_Framework_NETCore.DBHelper.DataBase();
            try
            {
                ...
                if (CAO == "Delete")
                {
                    sql = $"delete qcm_fittingsample_storage_m  where item_no='{ITEM_NO}'";
                }
                if (CAO == "Update")
                {
                    sql = $"update qcm_fittingsample_storage_m set location_no='{location_no}',location_name='{location_name}' where item_no='{ITEM_NO}'";
                }
        ...
        /// <param name="OBJ"></param>
        /// <returns></returns>
        public static SJeMES_Framework_NETCore.WebAPI.ResultObject FittingsamplestorageUpdateView(object OBJ)
        {
            SJeMES_Framework_NETCore.WebAPI.RequestObject ReqObj = (SJeMES_Framework_NETCore.WebAPI.RequestObject)OBJ;
            SJeMES_Framework_NETCore.WebAPI.ResultObject ret = new SJeMES_Framework_NETCore.WebAPI.ResultObject();
            SJeMES_Framework_NETCore.DBHelper.DataBase DB = new SJeMES_Framework_NETCore.DBHelper.DataBase();
            try
            {
                ...
                //转译
                //先找料品bom
                string sql = $@"select item_no,item_name,vend_name,prod_no,location_name,location_no from  qcm_fittingsample_storage_m where item_no='{item_no}'";
                DataTable dt = DB.GetDataTable(sql);
                dic.Add("data", dt);
                ret.IsSuccess = true;
        ...
        /// <param name="OBJ"></param>
        /// <returns></returns>
        public static SJeMES_Framework_NETCore.WebAPI.ResultObject FittingsamplestorageGetList(object OBJ)
        {
            SJeMES_Framework_NETCore.WebAPI.RequestObject ReqObj = (SJeMES_Framework_NETCore.WebAPI.RequestObject)OBJ;
            SJeMES_Framework_NETCore.WebAPI.ResultObject ret = new SJeMES_Framework_NETCore.WebAPI.ResultObject();
            SJeMES_Framework_NETCore.DBHelper.DataBase DB = new SJeMES_Framework_NETCore.DBHelper.DataBase();
            try
            {
                ...
                {
                    strwhere+= $@" and putin_date ='{putin_date}'";
                }
                if (!string.IsNullOrEmpty(end_date)&& string.IsNullOrEmpty(putin_date))
                {
                    strwhere += $@" and putin_date ='{end_date}'";
                }
                if (!string.IsNullOrEmpty(putin_date) && !string.IsNullOrEmpty(end_date))
                {
                    strwhere += $@"AND ( putin_date  BETWEEN '{putin_date}' AND '{end_date}') ";
                }
                sql = $@"select item_no,item_name,location_name,vend_name,prod_no,putin_date,end_date from  qcm_fittingsample_storage_m  where 1=1 {strwhere} order by id desc";
                Dictionary<string, object> dic = new Dictionary<string, object>();
        ...
        /// <param name="OBJ"></param>
        /// <returns></returns>
        public static SJeMES_Framework_NETCore.WebAPI.ResultObject FittingsamplestorageGetView(object OBJ)
        {
            SJeMES_Framework_NETCore.WebAPI.RequestObject ReqObj = (SJeMES_Framework_NETCore.WebAPI.RequestObject)OBJ;
            SJeMES_Framework_NETCore.WebAPI.ResultObject ret = new SJeMES_Framework_NETCore.WebAPI.ResultObject();
            SJeMES_Framework_NETCore.DBHelper.DataBase DB = new SJeMES_Framework_NETCore.DBHelper.DataBase();
...
select max(a.RCPT_DATE) from WMS_RCPT_M aa left
        ...
        /// <param name="OBJ"></param>
        /// <returns></returns>
        public static SJeMES_Framework_NETCore.WebAPI.ResultObject FittingsamplestorageGetViewS(object OBJ)
        {
            SJeMES_Framework_NETCore.WebAPI.RequestObject ReqObj = (SJeMES_Framework_NETCore.WebAPI.RequestObject)OBJ;
            SJeMES_Framework_NETCore.WebAPI.ResultObject ret = new SJeMES_Framework_NETCore.WebAPI.ResultObject();
            SJeMES_Framework_NETCore.DBHelper.DataBase DB = new SJeMES_Framework_NETCore.DBHelper.DataBase();
...
select max(a.RCPT_DATE) from WMS_RCPT_M aa left
        ...
        /// <param name="OBJ"></param>
        /// <returns></returns>
        public static SJeMES_Framework_NETCore.WebAPI.ResultObject FittingsamplestorageGetKuView(object OBJ)
        {
            SJeMES_Framework_NETCore.WebAPI.RequestObject ReqObj = (SJeMES_Framework_NETCore.WebAPI.RequestObject)OBJ;
            SJeMES_Framework_NETCore.WebAPI.ResultObject ret = new SJeMES_Framework_NETCore.WebAPI.ResultObject();
            SJeMES_Framework_NETCore.DBHelper.DataBase DB = new SJeMES_Framework_NETCore.DBHelper.DataBase();
            try
            {
                ...
                Dictionary<string, object> dic = new Dictionary<string, object>();
                //转译
                string sql = $@"select location_no,location_name from bdm_fittingsample_location where location_no='{location_no}'";
                DataTable dt = DB.GetDataTable(sql);
                if (dt.Rows.Count == 0)
                {
